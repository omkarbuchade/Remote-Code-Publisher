 ...
 * --------------------
 * ver 1.5 : 03 Feb 2019
 * - Added the functionality to store files that match a regular expression in a vector
 * - Added a method GetFiles() to return the vector of files that matched the regular expression
 * ver 1.4 : 24 Jan 2019
...
namespace FileSystem
{
  ...
  public:
	...
	using fname = std::vector<std::string>;
  ...
  */
  inline void DirExplorerN::find(const std::string& path)
  {
    ...
    for (auto patt : patterns_)
    {
      ...
      for (auto f : files)
      {
		...
		fileProcess_.push_back(fpath + "\\" + f);
      }
    }
  ...
  inline DirExplorerN::fname DirExplorerN::GetFiles()
  {
	  return fileProcess_;
  }
}